    ...
    def forward(self, graph, feat, edge_weight=None, get_attention=False):
        ...
        with graph.local_scope():
            ...
            graph.edata["a"] = self.attn_drop(
                dglnn.functional.edge_softmax(graph, e)
            )  # (num_edge, num_heads)
            ################
...
class GATv2(nn.Module):
    ...
    def forward(self, blocks, inputs):
        ...
        for l, block in enumerate(blocks):
            ...
            block.srcdata["embed_norm"] = th.reshape(
                th.norm(h, dim=1, keepdim=True), (-1,)
            )
            h, a = self.gatv2_layers[l](
                block,
                h,
                edge_weight=(
                    block.edata["edge_weights"]
    ...
    def inference(self, g, device, batch_size, use_uva, num_workers):
        ...
        g.ndata["h"] = g.ndata["features"].bfloat16()
...
class GCN(nn.Module):
    ...
    def init(self, in_feats, n_hidden, n_classes, n_layers, activation, dropout):
        ...
        if n_layers > 1:
            self.layers.append(
                dglnn.GraphConv(
                    in_feats, n_hidden, activation=activation, allow_zero_in_degree=True
                )
            )
            for i in range(1, n_layers - 1):
                self.layers.append(
            ...
            self.layers.append(
                dglnn.GraphConv(in_feats, n_classes, allow_zero_in_degree=True)
            )
        self.dropout = nn.Dropout(dropout)
        self.activation = activation
