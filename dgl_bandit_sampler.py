...
class BanditSampler(dgl.dataloading.BlockSampler): # consider to use unbiased node embedding and edge_weights
    def __init__(self, nodes_per_layer, importance_sampling=True, weight='w', out_weight='edge_weights', node_embedding='nfeat', replace=False, eta=0.4, num_epochs=1):
        ...
        self.eta = eta
        self.T = num_epochs
        self.exp3_weights = None # None then intialize inside the code (if)
        self.exp3_prob = None

    def exp3(self, g, seed_nodes):
        ...
        # neighbor_nodes = insg.out_edges(seed_nodes)[1].unique()
        # exp3_prob[neighbor_nodes] = 1/len(neighbor_nodes) # no need to intialize it
        ...
        # self.update_probability(insg, self.exp3_prob, chosen_nodes, seed_nodes, rewards, self.exp3_weights, self.eta, num_nodes_to_sample, insg.num_nodes(), self.T)
    ...
    def compute_prob(self, g, seed_nodes, weight, optimal=False):
        """
        g : the whole graph
        ...
        if optimal:
          ...
          # print('weight', weight)
          # get the node data from the subgraph
          node_data = insg.ndata[self.node_embedding][insg.ndata[dgl.NID].long()]
          # get the norm
          node_data_norm = torch.norm(node_data, dim=1, keepdim=True)
          node_data_norm = torch.reshape(node_data_norm, (-1,))
          # multiply weight (alpha) by norm of embedding
          weight_by_norm = dgl.ops.e_mul_v(insg, weight, node_data_norm)
          # weight_sum = dgl.ops.copy_e_sum(out_frontier, weight_by_norm)
          # #
          # weight_div_sum = dgl.ops.e_div_v(out_frontier, weight_by_norm, weight_sum)
          prob = dgl.ops.copy_e_sum(out_frontier, weight_by_norm ** 2)
        ...
        if self.importance_sampling:
          ...
          # get the weights of the subgraph edges
          weight = weight[out_frontier.edata[dgl.EID].long()] # calc q_ij
          # print('weightweightweight', weight)
          # print('weight', weight)
          # 
    ...
    def calculate_reward(self, insg, q):
        ...
        rewards = dgl.ops.e_div_u(insg, alpha ** 2, k * (q ** 2))
        # Compute reward as element-wise product of edge weight and node L2 norm squared
        rewards = dgl.ops.e_mul_u(insg, rewards, h_j_norm ** 2)
